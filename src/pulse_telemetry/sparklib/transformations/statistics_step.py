...
statistics_step_schema = T.StructType(
    [
        ...
        T.StructField("metadata", dataType=T.MapType(T.StringType(), T.DoubleType()), nullable=True),  # First non-null metadata entry over the step
        # Metadata
...
def statistics_step(df: "DataFrame") -> "DataFrame":
    ...
    time_weighted_avg = lambda col: (F.sum(F.col(col) * F.col("duration__s")) / F.sum("duration__s"))
    ...
    return df.groupBy(
        ...
        # Time (duration calculated from difference, see below)
        ...
        F.min(F.when(F.col("current__A") < 0, F.col("current__A"))).alias("min_discharge_current__A"),
        F.max(F.when(F.col("current__A") > 0, F.col("current__A"))).alias("max_charge_current__A"),
        F.max(F.when(F.col("current__A") < 0, F.col("current__A"))).alias("max_discharge_current__A"),
        ...
        F.min(F.when(F.col("power__W") < 0, F.col("power__W"))).alias("min_discharge_power__W"),
        F.max(F.when(F.col("power__W") > 0, F.col("power__W"))).alias("max_charge_power__W"),
        F.max(F.when(F.col("power__W") < 0, F.col("power__W"))).alias("max_discharge_power__W"),
        ...
        # Resolution diagnostics
        F.max("voltage_delta__V").alias("max_voltage_delta__V"),
        F.max("current_delta__A").alias("max_current_delta__A"),
        ...
        "duration__s", (F.unix_timestamp("end_time") - F.unix_timestamp("start_time")).cast(T.DoubleType())
    )
