...
@pytest.fixture(scope="session")
def load_container_image(build_docker_image, kind_cluster):
    subprocess.run(["kind", "load", "docker-image", "pulse-telemetry:latest", "--name", "kind"], check=True)
    yield


@pytest.fixture(scope="session")
def kubernetes_services(load_container_image):
